...
7. **Optionally fill the form in the browser** - with Playwright MCP available (browsermcp as fallback; install and wiring in `dependency.md`) it clears the form, opens the prefilled URL, types the manual fields and verifies them; it stops before the PRIVACY checkbox and never clicks Submit. Two workflows: **normal** (every check, end screenshot) and **lean** (one read at the end, fewer tokens), chosen by `FILL_MODE` in `.env` or per run with `lean: on`.
...
|-- SKILL.md                 # Claude Code entry point (workflow + rules, lean mode)
|-- .env.sample              # copy to .env and fill once (.env itself is gitignored)
|-- dependency.md            # browser MCP install and wiring (Playwright first, browsermcp fallback)
`-- references/
    |-- form_fields.md       # verified field spec + prefill map + browser workflows
    |-- act_options.md       # the 40 Act of Advocacy entries, verbatim
    `-- product_options.md   # the 1097 Product entries, verbatim (grep only)
```
